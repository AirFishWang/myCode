...
int main(int argc, char** argv)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// section1 利用反射平面绘制模板
        glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);   //rgba均不显示
		...
		glBindVertexArray(planeVAOId);
		glDrawArrays(GL_TRIANGLES, 0, 6);  //由于设置glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE)
		...
		// 绘制第一个立方体反射
		glm::mat4 firstReflectModel;    //注意这里是右乘矩阵，真正的变换是相反的
