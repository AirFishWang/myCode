...
#pragma endregion
    ...
    while (!glfwWindowShouldClose(window))
    {
        /*
        关于优化问题：
            先绘制物体，更新深度，然后在绘制背景，但是在绘制背景的时候进行前置深度测试(即在片元着色器之前把被遮挡的片元抛弃掉)。
        */
        ...
        // skybox cube
        glBindVertexArray(skyboxVAO);
        //glActiveTexture(GL_TEXTURE0);                                          //注释掉不影响结果
        //glUniform1i(glGetUniformLocation(skyboxShader.Program, "skybox"), 0);  //注释掉不影响结果
