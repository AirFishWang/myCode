...
{  
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
  //glProgramUniform1i(program_, glGetUniformLocation(program_, "sRGB"), 1);     //将1号纹理单元作为整数传递给片元着色器，片元着色器中使用uniform变量sRGB对应这个纹理采样器
  glUniform1i(glGetUniformLocation(program_, "sRGB"), 1);
  ...
  // Timer queries.
  //glGenQueries(kNumQueries, query_ids_);      //遮挡查询，生成未使用的查询ID
}
...
void Renderer::render(const float* const coeffs_data, cv::Mat & output,
  ...
  *draw_time = costTime;
  ...
  costTime = tmpTime.duration();
  *readback_time = costTime;
  LOGE("JNI: *************************************************glReadPixels cost %lf ms", costTime);             
...
/*
  glProgramUniformMatrix3x4fv(program_,glGetUniformLocation(program_, "uGuideCcm"),1, false, ccm);
  glProgramUniform4fv(program_,glGetUniformLocation(program_, "uMixMatrix"),1, mix_matrix);
  glProgramUniform3fv(program_,glGetUniformLocation(program_, "uGuideShifts"),16, shifts);
  glProgramUniform3fv(program_,glGetUniformLocation(program_, "uGuideSlopes"),16, slopes);*/
